...
app.post('/api/categories', adminAuth, async (req, res) => {
  try {
    const cat = new Category({ name: req.body.name, description: req.body.description || '' });
    await cat.save();
...
app.post('/api/submit-vote', async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Your vote is awaiting admin approval.', submission });
  } catch (e) { res.status(500).json({ message: e.message }); }
});
...
app.post('/api/approve/:id', adminAuth, async (req, res) => {
  try {
    ...
    if (sub.status === 'approved') return res.status(400).json({ message: 'Already approved' });

    sub.status = 'approved';
    await sub.save();

    // Add votes to candidate
    await Candidate.findByIdAndUpdate(sub.candidateId, { $inc: { votes: sub.votes } });
    res.json({ success: true, message: `Approved! ${sub.votes} vote(s) added to ${sub.candidateName}.` });
  } catch (e) { res.status(500).json({ message: e.message }); }
});

app.post('/api/reject/:id', adminAuth, async (req, res) => {
  try {
    const sub = await Submission.findByIdAndUpdate(req.params.id, { status: 'rejected' }, { new: true });
    if (!sub) return res.status(404).json({ message: 'Submission not found' });
...
app.get('/api/results', async (req, res) => {
  try {
    ...
    const results = categories.map(cat => ({
      category: cat,
      candidates: candidates.filter(c => c.category === cat).sort((a, b) => b.votes - a.votes),
    }));
    res.json(results);
  } catch (e) { res.status(500).json({ message: e.message }); }
});
...
// ─── CONNECT & START ──────────────────────────────────────────
mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/university-voting')
  .then(() => {
